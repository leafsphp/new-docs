...
Default checks look for a `username` field and test it against the [ValidUsername](v/2.0/core/forms?id=validate) rule, an `email` against the [email](v/2.0/core/forms?id=validate) rule and a `password` field against the [required](v/2.0/core/forms?id=validate) rule.
...
### [Leaf Authentication Methods](v/2.0/core/authentication) <sup><span style="background: rgb(11, 200, 70); color: white; padding: 3px 7px; font-size: 14px;">New in v2</span></sup>
...
Read [authentication](v/2.0/core/authentication) for more info

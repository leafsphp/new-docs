...
Default checks look for a `username` field and test it against the [ValidUsername](v/2.2-beta/core/forms?id=validate) rule, an `email` against the [email](v/2.2-beta/core/forms?id=validate) rule and a `password` field against the [required](v/2.2-beta/core/forms?id=validate) rule.
...
### [Leaf Authentication Methods](v/2.2-beta/core/authentication)
...
Read [authentication](v/2.2-beta/core/authentication) for more info

...
Default checks look for a `username` field and test it against the [ValidUsername](leaf/v/lucky-charm/core/forms?id=validate) rule, an `email` against the [email](leaf/v/lucky-charm/core/forms?id=validate) rule and a `password` field against the [required](leaf/v/lucky-charm/core/forms?id=validate) rule.
...
### [Leaf Authentication Methods](leaf/v/lucky-charm/core/authentication)
...
Read [authentication](leaf/v/lucky-charm/core/authentication) for more info

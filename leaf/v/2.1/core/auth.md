...
Default checks look for a `username` field and test it against the [ValidUsername](leaf/v/2.1/core/forms?id=validate) rule, an `email` against the [email](leaf/v/2.1/core/forms?id=validate) rule and a `password` field against the [required](leaf/v/2.1/core/forms?id=validate) rule.
...
### [Leaf Authentication Methods](leaf/v/2.1/core/authentication)
...
Read [authentication](leaf/v/2.1/core/authentication) for more info

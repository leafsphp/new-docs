...
As explained [before](leaf/v/2.4-beta/intro/htaccess), Leaf uses a single root file, to which all the server requests are redirected. Leaf then takes these requests and matches them to rules you have defined. The results are then displayed to the user. It's actually a very simple concept.
...
Using Post Params
View [Request](leaf/v/2.4-beta/http/request) for more info on handling params
...
This section assumes you've read [working with controllers](leaf/v/2.4-beta/routing/controller). In an MVC application, controllers play a major role as they're the bridge between your view and your model.
...
Resource routes are handled by a [resource controller](leaf/v/2.4-beta/routing/controller?id=resource-controller).
